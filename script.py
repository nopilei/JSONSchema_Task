...
def main():
    ...
    for event_filename, event in events.items():
        if event is None:
            logger.info(f"'{event_filename} has not corresponding json schema!")
        else:
            event_name = event.get('event', None)
        if event_name is None:
            logger.info(f"Field 'event' is absent in '{event_filename}'!")
            ...
            try:
                ...
                logger.info(f"Validation error when tried to apply"
